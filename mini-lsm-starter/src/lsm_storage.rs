...
impl LsmStorageInner {
            ...
            for record in records {
                match record {
                    ManifestRecord::Flush(sst_id) => {
                        let res = mem_tables.remove(&sst_id);
                    ...
                    ManifestRecord::Compaction(task, output) => {
                        ...
                        next_sst_id = std::cmp::max(
                            next_sst_id,
                            output.iter().max().copied().unwrap_or_default(),
                        );
                    }
                    ManifestRecord::NewMemtable(id) => {
                        next_sst_id = std::cmp::max(next_sst_id, id);
                        mem_tables.insert(id);
                    }
                }
            }
                ...
                .chain(state.levels.iter().flat_map(|(_, files)| files))
            {
                ...
                let sst = SsTable::open(
                    ...
                    FileObject::open(&Self::path_of_sst_static(path, table_id))
                        .with_context(|| format!("fail to open SST: {}", table_id))?,
                )?;
    ...
    pub fn get(&self, _key: &[u8]) -> Result<Option<Bytes>> {
        ...
        let mut level_iters = Vec::with_capacity(state.levels.len());
        for (_, level_sst_ids) in &state.levels {
            let mut level_sst = Vec::with_capacity(level_sst_ids.len());
    ...
    fn try_freeze(&self, esitimate_size: usize) -> Result<()> {
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        ...
        let next_memtable = if self.options.enable_wal {
            Arc::new(MemTable::create_with_wal(
                next_memtable_id,
                self.path_of_wal(next_memtable_id),
            )?)
        } else {
    ...
    pub fn scan(
        ...
        for (_, level_sst_ids) in &snapshot.levels {
            ...
            let level_iter = match _lower {
                Bound::Included(key) => {
                    SstConcatIterator::create_and_seek_to_key(level_sst, KeySlice::from_slice(key))?
                }
                Bound::Excluded(key) => {
