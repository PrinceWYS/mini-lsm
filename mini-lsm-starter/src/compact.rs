...
impl LsmStorageInner {
    ...
    fn compact(&self, _task: &CompactionTask) -> Result<Vec<Arc<SsTable>>> {
        ...
        match _task {
            CompactionTask::ForceFullCompaction {
                ...
                let mut iters =
                    Vec::with_capacity(snapshot.l0_sstables.len() + snapshot.levels[0].1.len());
                for sst in snapshot
                    .l0_sstables
                    .iter()
            ...
            | CompactionTask::Simple(SimpleLeveledCompactionTask {
                ...
                Some(_) => {
                    ...
                    for id in upper_level_sst_ids.iter() {
                        upper_ssts.push(snapshot.sstables[id].clone());
                    }
    ...
    pub fn force_full_compaction(&self) -> Result<()> {
        ...
        let comnpaction_task = CompactionTask::ForceFullCompaction {
            l0_sstables: l0_sstables.clone(),
            l1_sstables: l1_sstables.clone(),
        };

        // compact
        let sstables = self.compact(&comnpaction_task)?;
        {
